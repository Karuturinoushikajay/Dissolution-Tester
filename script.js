...
var PROCEDURE_CALIBRATION_TEMPERATURE = [
    '1. Allow the bath sensor to stabilize near the operating temperature.',
    '2. Measure Bath temperature with a certified reference thermometer.',
    '3. Enter the Actual (reference) value for Bath (valid range 15–55 °C).',
    '4. Press Calibrate to apply the correction to the instrument.',
    '5. Confirm that the live bath reading matches the reference after calibration completes.'
];
...
function _populateAuditFilterDropdowns(userEl, actionEl, fullList) {
    ...
    var coreActions = [
        ...
        'Audit log viewed', 'Print A4', 'Print thermal',
...
function runTemperatureCalibration() {
    ...
    if (!isValidTemp(bathActual)) {
        if (typeof showAppModal === 'function') {
            showAppModal(
                'Please enter an Actual bath temperature between ' + TEMP_CAL_MIN + ' and ' + TEMP_CAL_MAX + ' °C.',
                'Calibration'
            );
        }
        return;
    ...
    var cmd = 'temp,calibrate,' + bathTxt;
    ...
    apiRequest(API_BASE + '/api/hardware/command', {
        ...
        if (result && result.ok !== false && !result.error) {
            ...
            if (typeof logAuditEvent === 'function') {
                logAuditEvent('Temperature calibration completed', bathTxt + ' °C', {
                    eventType: 'lifecycle',
                    entityType: 'calibration'
                });
            }
