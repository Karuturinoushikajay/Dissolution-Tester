...
AUDIT_ACTION_GROUP_FILTERS: Dict[str, List[str]] = {
    ...
    "Calibration": [
        "(COALESCE(action, '') IN ('Temperature calibration started', 'Temperature calibration completed', 'Hardware calibration'))",
        "(COALESCE(action, '') IN ('Entered screen', 'Exited screen') AND ("
        "COALESCE(details, '') LIKE '%Calibration%' OR COALESCE(details, '') LIKE '%Load Calibration%' OR "
        "COALESCE(details, '') LIKE '%Distance Calibration%' OR COALESCE(details, '') LIKE '%Select Calibration Type%'))",
    ],
    "Cleaning Cycle": [
